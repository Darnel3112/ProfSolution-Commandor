...
Процедура Печать(ИдентификатораКоманды,ОбъектыНазначения) Экспорт
	...
	ОткрытьФорму("ВнешняяОбработка.ВнешняяПечатнаяФормаПечатиЭтикеток.Форма",ПараметрыФормы,ОбъектыНазначения[0],,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура КомандаПечать(Команда)
	...
	Иначе
		...
		Сообщение.Текст = "Поле организация не может быть пустым!";
