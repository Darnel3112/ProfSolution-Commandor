...
Функция СформироватьПечатнуюФорму(МассивОбъектов, ОбъектыПечати)
	...
	// {{ ПРОФРЕШЕНИЕ, Пелых К.Д. - 17.07.2023 - #УправлениеАктивами
	КонтактныйТелефон = Новый Запрос(	
	"ВЫБРАТЬ
				...
				Пока ГруппировкаРабочееМестоДо.Следующий() Цикл
					...
					Пока ГруппировкаРабочееМестоПосле.Следующий() Цикл
						...
						Пока ГруппировкаМОЛПосле.Следующий() Цикл
							...
							ФизЛицо = СдалОборудование;
							Пользователь = ПР_ОбщегоНазначенияСервер.ПолучитьПользователяПоФизЛицу(ФизЛицо);  
							...
							КонтактныйТелефон.УстановитьПараметр("ФизЛицо", СдалОборудование);
							Выборка = КонтактныйТелефон.Выполнить().Выбрать();
							Выборка.Следующий(); 
							Телефон = "+7 " + ЛЕВ(Выборка.Телефон,3) + "-" + СРЕД(Выборка.Телефон,4,3) + "-" + СРЕД(Выборка.Телефон,7,2) + "-" + ПРАВ(Выборка.Телефон,2); 
							ОбластьМакетаКонтактныйТелефон.Параметры.ТелефонОтправителя = Телефон;
							
							ФизЛицо = ГруппировкаМОЛПосле.МОЛПосле; 
							Пользователь = ПР_ОбщегоНазначенияСервер.ПолучитьПользователяПоФизЛицу(ФизЛицо);
							Если ЗначениеЗаполнено(Пользователь) Тогда  
								ОбластьМакетаПодвал.Параметры.ДолжностьПринял = Пользователь.Должность;
							КонецЕсли;
							ОбластьМакетаПодвал.Параметры.Принял = ГруппировкаМОЛПосле.МОЛПосле;
							
							КонтактныйТелефон.УстановитьПараметр("ФизЛицо", ГруппировкаМОЛПосле.МОЛПосле);
							Выборка = КонтактныйТелефон.Выполнить().Выбрать();
							Выборка.Следующий();
							Телефон = "+7 " + ЛЕВ(Выборка.Телефон,3) + "-" + СРЕД(Выборка.Телефон,4,3) + "-" + СРЕД(Выборка.Телефон,7,2) + "-" + ПРАВ(Выборка.Телефон,2); 
							ОбластьМакетаКонтактныйТелефон.Параметры.ТелефонПолучателя = Телефон;			
							// }} ПРОФРЕШЕНИЕ, Пелых К.Д. - 17.07.2023 - #УправлениеАктивами
