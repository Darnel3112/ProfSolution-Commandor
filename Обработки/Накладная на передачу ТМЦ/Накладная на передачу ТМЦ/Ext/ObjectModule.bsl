				...
				Пока ГруппировкаРабочееМестоДо.Следующий() Цикл
					...
					Пока ГруппировкаРабочееМестоПосле.Следующий() Цикл
						...
						Пока ГруппировкаМОЛПосле.Следующий() Цикл
							...
							Пользователь = Справочники.Пользователи.НайтиПоНаименованию(Строка(ОбъектСсылка.ПР_СдалОборудованиеВЭксплуатацию));
							
							ОбластьМакетаПодвал.Параметры.ДолжностьСдал = Пользователь.Должность; 
							ОбластьМакетаПодвал.Параметры.Сдал = ОбъектСсылка.ПР_СдалОборудованиеВЭксплуатацию; 
							
							Пользователь = Справочники.Пользователи.НайтиПоНаименованию(Строка(ГруппировкаМОЛПосле.МОЛПосле));
